...
The patch around `GeneralPacketHandler.HandleInventoryDoubleUpdate` is worth keeping as a compatibility probe. The decompiled vanilla path looked suspicious for inventories where `InventoryId1 != InventoryId2`. If this is truly a vanilla bug, ResponsiveVS should either fix it narrowly or include a verified test case.
