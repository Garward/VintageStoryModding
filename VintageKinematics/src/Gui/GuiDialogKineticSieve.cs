...
{
    public class GuiDialogKineticSieve : GuiDialogBlockEntity
    {
        public override double DrawOrder => 0.2;

        public GuiDialogKineticSieve(string title, InventoryBase inventory, BlockPos pos, ICoreClientAPI capi)
        ...
        private void ComposeDialog(string title)
        {
            ...
            string inputLabel = Lang.Get("vintagekinematics:kineticsieve-input");
            string outputLabel = Lang.Get("vintagekinematics:kineticsieve-outputs");
            int[] inputSlot = new[] { 0 };
            int[] outputSlots = new[] { 1, 2, 3, 4, 5, 6, 7, 8, 9 };

            SingleComposer = capi.Gui.CreateCompo("kineticsieve-" + BlockEntityPosition, dialogBounds)
